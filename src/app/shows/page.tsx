...
export default function ShowsDashboard() {
  ...
  const filteredItems = items.filter((i) => {
    ...
    return isShow && matchesSearch && matchesWatchFilter;
  }).sort((a, b) => {
    let result = 0;
    if (sortBy === "watched") {
        ...
        <section>
          ...
          {filteredItems.length === 0 ? (
            <p className="text-neutral-500 italic bg-neutral-900/50 p-8 rounded-xl border border-neutral-800 text-center">
              {searchQuery ? "No results match your search." : `No TV Shows found in your recent history.`}
            </p>
          ) : (
            <>
              <div className={displayView === "grid" ? "grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-6" : "relative flex flex-col gap-4 lg:gap-6 w-full items-stretch"}>
